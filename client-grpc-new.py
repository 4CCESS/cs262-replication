...
class ChatClientGUI:
    def __init__(self, root, delivery_address):
        self.root = root
        self.root.title("Chat Client")
        self.delivery_address = delivery_address

        print("[DEBUG] Starting ChatClientGUI with delivery_address:", delivery_address)

        # Queue for incoming messages from client process.
        self.queue = queue.Queue()
        # Start the client process with the delivery address.
        self.client_process = subprocess.Popen(
            ["./client", self.delivery_address],
    ...
    def send_authentication(self):
        ...
        if username and password:
            # Build the command with the mode and credentials.
    ...
    def open_main_window(self):
        ...
        # Offline message retrieval controls.
        ...
        # Bottom pane: send message controls.
    ...
    def send_message(self):
        ...
        if recipient and message:
            # Queue the outgoing message locally for ACK tracking.
    ...
    def logout(self):
        ...
        for widget in self.root.winfo_children():
            widget.destroy()
        self.__init__(self.root, self.delivery_address)
    ...
    def process_queue(self):
        while not self.queue.empty():
            ...
            if data.get("type") == "confirmation":
                ...
                print("[DEBUG] Received history message:", data)
                messages = data.get("messages", [])
                for msg in messages:
                    display_str = f"[{msg.get('sender')}][{msg.get('message_id')}]: {msg.get('message')}"
                    # Right justify if sent by current user.
                    if msg.get("sender") == self.current_user:
                        display_str = display_str.rjust(80)
                    self.message_listbox.insert(tk.END, display_str)
                    self.messages.append(msg)
                print("[DEBUG] Loaded history messages:", messages)
            elif data.get("type") in ("chat", "history"):
                self.messages.append(data)
                display_str = f"[{data.get('sender')}][{data.get('message_id')}]: {data.get('message')}"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(f"Usage: python3 {sys.argv[0]} <delivery_address>")
        sys.exit(1)
    delivery_address = sys.argv[1]
    root = tk.Tk()
    app = ChatClientGUI(root, delivery_address)
    root.mainloop()
